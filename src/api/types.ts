// Shapes the server returns. These mirror server/app/schemas.py; if you change
...
export interface PairedDevice extends Device {
  // Readable exactly once, when the device is paired.
  token: string;
  setup: Record<string, string>;
}

// A sensor that is not wired reads null, never 0. The dashboard shows that as
// "no sensor" rather than inventing a value.
...
// The leaf model's verdict, as codes; the client owns the wording.
...
export interface Capture {
  ...
  // straight from the model on the greenhouse node; score and level are null
  // when the node could not read the photo (night, no leaf in view)
...
// kind 'camera' is "take a photo now": the greenhouse camera takes it, not
// the phone.
...
// Pushed over the websocket when a node reports.
